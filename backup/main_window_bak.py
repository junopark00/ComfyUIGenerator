...
#         )
    ...
    def create_layout(self):
        ...
        # 왼쪽 옵션 영역이 너무 좁아지지 않도록 QWidget으로 감싸고 최소 넓이 지정
    ...
    async def on_generate(self):
        ...
        os.makedirs(os.path.dirname(self.path_to_save_le.text()), exist_ok=True)
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as session:
                ...
                # elif self.mode == "Trellis2":
                if self.mode == "Trellis2":
                    # Trellis2: 결과 파일이 이미 save_path에 저장됨. 해당 경로에서 glb 파일을 찾아 바로 로드
                    ...
                    self.append_info_log(f"Mesh file Loaded: {os.path.basename(save_path)}")
